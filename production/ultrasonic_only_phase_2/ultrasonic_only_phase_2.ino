/*
  ...
  Operation:    The claw detects whether it is going up or down and determines the time to open/close.
                When the claw is rising, it must reach the set threshold in order for the claw to be triggered.
                Once triggered, the claw detects if it has started decending, and once it stays at a certain height for
                more than a set time, it will automatically close. The LED indicators are added to help indicate
                the stage the claw is in.
 */
...
/* WARNING: Enabling DEBUG macro will cause the White LED to not be functional. (because it is connected on the serial pins.)
  ...
  #define DISTANCE_THRESHOLD      50
      //the distance detected to trigger the claw to start detection
  ...
  #define CLAW_DELAY              2000  //the time delay before the claw opens or closes
  ...
  #define ALLOWED_ERROR           2     //the amount of error in cm allowed for the ultrasonic sensor to determine the claw has stopped moving
...
void ClawDelay () {
  //stores current time
  long time = millis();
  int last_ultrasonic = GetUltrasonic();
  int current_ultrasonic = GetUltrasonic();
  ...
  while (millis() < time + CLAW_DELAY) {
    last_ultrasonic = current_ultrasonic;
    current_ultrasonic = GetUltrasonic();

    if ((current_ultrasonic > last_ultrasonic + ALLOWED_ERROR) || (current_ultrasonic < last_ultrasonic - ALLOWED_ERROR)) {
      time = millis();
    }
  }
